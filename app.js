...
fr: {
title: “Mon Livre de Recettes”, search: “Rechercher…”, all: “Tout”, add: “Ajouter une recette”,
edit: “Modifier”, delete: “Supprimer”, save: “Enregistrer”, cancel: “Annuler”,
recipeName: “Nom de la recette”, category: “Catégorie”, prepTime: “Temps de prép.”,
cookTime: “Temps de cuisson”, servings: “Portions”, ingredients: “Ingrédients”,
...
noMatch: “Aucune recette ne correspond.”, confirmDelete: “Supprimer cette recette ?”,
...
<div>
<label style={labelStyle}>{label}</label>
<textarea value={value ?? “”} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={rows} style={{…inputStyle, resize:“vertical”, lineHeight:1.6}} />
</div>
);
}
  ...
  <div style={{maxWidth:900, margin:"0 auto", padding:"24px 16px"}}>
      ...
      <div>
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(260px, 1fr))", gap:20}}>
            {filtered.map(recipe => (
              <div key={recipe.id} onClick={() => openDetail(recipe)} style={{background:"#fff", borderRadius:16, overflow:"hidden", boxShadow:"0 2px 12px rgba(0,0,0,0.08)", cursor:"pointer", border:"1px solid #ece5d8"}}>
    ...
    {!loading && view === "detail" && selectedRecipe && (
      <div style={{background:"#fff", borderRadius:20, overflow:"hidden", boxShadow:"0 4px 20px rgba(0,0,0,0.10)", border:"1px solid #ece5d8"}}>
        <div style={{background:catColor(selectedRecipe.category), padding:"32px 32px 24px", display:"flex", justifyContent:"space-between", alignItems:"flex-start"}}>
          <div>
            <div style={{fontSize:48, marginBottom:8}}>{catIcon(selectedRecipe.category)}</div>
            <h2 style={{margin:0, color:"#fff", fontSize:26, lineHeight:1.2}}>{displayName(selectedRecipe)}</h2>
            {lang==="en" && selectedRecipe.nameFr && <p style={{margin:"4px 0 0", color:"rgba(255,255,255,0.75)", fontSize:15, fontStyle:"italic"}}>{selectedRecipe.nameFr}</p>}
            <span style={{display:"inline-block", marginTop:10, background:"rgba(255,255,255,0.2)", borderRadius:20, padding:"3px 14px", color:"#fff", fontSize:13}}>{displayCat(selectedRecipe)}</span>
          </div>
          <div style={{display:"flex", gap:8}}>
            <button onClick={() => openEdit(selectedRecipe)} style={{background:"rgba(255,255,255,0.2)", color:"#fff", border:"none", borderRadius:8, padding:"8px 16px", cursor:"pointer", fontFamily:"Georgia, serif", fontSize:14}}>✏️ {t.edit}</button>
            <button onClick={() => setDeleteConfirm(selectedRecipe.id)} style={{background:"rgba(255,255,255,0.2)", color:"#fff", border:"none", borderRadius:8, padding:"8px 16px", cursor:"pointer", fontFamily:"Georgia, serif", fontSize:14}}>🗑️</button>
          </div>
        </div>
        <div style={{padding:"24px 32px"}}>
          <div style={{display:"flex", gap:24, marginBottom:28, flexWrap:"wrap"}}>
            {selectedRecipe.prepTime && <div style={{textAlign:"center"}}><div style={{fontSize:22}}>⏱️</div><div style={{fontSize:13, color:"#9a8a6a"}}>{t.prepTime}</div><div style={{fontWeight:"bold", fontSize:16}}>{selectedRecipe.prepTime} {t.min}</div></div>}
            {selectedRecipe.cookTime && <div style={{textAlign:"center"}}><div style={{fontSize:22}}>🔥</div><div style={{fontSize:13, color:"#9a8a6a"}}>{t.cookTime}</div><div style={{fontWeight:"bold", fontSize:16}}>{selectedRecipe.cookTime} {t.min}</div></div>}
            {selectedRecipe.servings && <div style={{textAlign:"center"}}><div style={{fontSize:22}}>👥</div><div style={{fontSize:13, color:"#9a8a6a"}}>{t.servings}</div><div style={{fontWeight:"bold", fontSize:16}}>{selectedRecipe.servings}</div></div>}
          </div>
          {displayIngredients(selectedRecipe) && (
            <div style={{marginBottom:24}}>
              <h4 style={{color:catColor(selectedRecipe.category), fontSize:16, marginBottom:12, borderBottom:`2px solid ${catColor(selectedRecipe.category)}`, paddingBottom:6}}>🛒 {t.ingredients}</h4>
              <ul style={{margin:0, paddingLeft:20}}>
                {displayIngredients(selectedRecipe).split("\n").filter(Boolean).map((ing, i) => <li key={i} style={{marginBottom:6, fontSize:15, lineHeight:1.5}}>{ing}</li>)}
              </ul>
            </div>
          )}
          {displayInstructions(selectedRecipe) && (
            <div style={{marginBottom:24}}>
              <h4 style={{color:catColor(selectedRecipe.category), fontSize:16, marginBottom:12, borderBottom:`2px solid ${catColor(selectedRecipe.category)}`, paddingBottom:6}}>📋 {t.instructions}</h4>
              <div style={{lineHeight:1.8, fontSize:15, whiteSpace:"pre-line"}}>{displayInstructions(selectedRecipe)}</div>
            </div>
          )}
          {displayNotes(selectedRecipe) && (
            <div style={{background:"#faf7f2", borderRadius:10, padding:"14px 18px", borderLeft:`4px solid ${catColor(selectedRecipe.category)}`}}>
              <h4 style={{margin:"0 0 6px", fontSize:14, color:"#9a8a6a"}}>💡 {t.notes}</h4>
              <p style={{margin:0, fontSize:14, lineHeight:1.6, fontStyle:"italic"}}>{displayNotes(selectedRecipe)}</p>
            </div>
          )}
        </div>
      </div>
